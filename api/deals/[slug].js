...
// Product structured data is only valid (per Google's requirements) when it
// includes at least one of offers/review/aggregateRating. If the live price
// fetch failed or returned nothing, we have no offers data to report, so we
// omit the Product schema entirely rather than emit an incomplete block that
// Search Console flags as an invalid rich result item.
const productSchema = cheapestPrice ? `<script type="application/ld+json">
${JSON.stringify({
